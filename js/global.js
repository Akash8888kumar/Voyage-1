/* Voyage One — shared frontend behavior */
...
document.addEventListener('DOMContentLoaded', function () {
  if (window.AOS) {
    AOS.init({ duration: 850, easing: 'ease-out-cubic', once: true, offset: 80, disable: 'mobile' });
  }
...
/* Home hero slider — 9 Unsplash images, 3-second autoplay + line controls */
(function(){
  ...
  const show=(next,user=false)=>{
    ...
    window.setTimeout(()=>{ slider.style.backgroundImage=`url("${images[index]}")`; slider.classList.remove('is-changing'); },180);
    updateControls();
    if(user) restart();
  };
  ...
  const restart=()=>{stop(); if(!reduced.matches) timer=window.setInterval(()=>show(index+1),3000);};
...
(function(){
  ...
  const mediaItems=()=>Array.from(document.querySelectorAll('[data-lightbox-type][data-lightbox-src]'));
  let current=0;
  let lastFocus=null;
